...
def build_date_fast_tree(file_path: str,
                         batch_size: int,
                         date_col: str) -> tuple:
...
def materialize_filtered_indices(file_path: str,
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        n_rows = len(batch)
        df     = batch.to_pandas()
        local_ix = [i for i in range(n_rows)
...
if __name__ == "__main__":
    ...
    lookup_metrics = measure_query_execution(
        lambda: fast_tree
            .rangeSearch(start_int, end_int, cpp_entries)
            .entries
    )
    ...
    con, sql_q4 = prepare_duckdb(filtered_orders, QUERY_PATH)
    engine_metrics_duck = measure_query_duckdb(4, con, sql_q4)
    ...
    ctx, sql_q4_df = prepare_datafusion(filtered_orders, QUERY_PATH)
    engine_metrics_df = measure_query_datafusion(4, ctx, sql_q4_df)
