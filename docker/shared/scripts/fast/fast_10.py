...
def prepare_duckdb(filtered_orders: pd.DataFrame, query_file: str):
    dest = "../data/tpch/parquet/filtered_orders.parquet"
    filtered_orders.to_parquet(dest, index=False, engine="pyarrow")

    con = duckdb.connect(":memory:")
    con.execute(f"CREATE TABLE customer AS SELECT * FROM read_parquet('{FILE_CUSTOMER}')")
    con.execute(f"CREATE TABLE orders   AS SELECT * FROM read_parquet('{dest}')")
...
def prepare_datafusion(filtered_orders: pd.DataFrame, query_file: str):
    ...
    ctx.register_parquet("orders",   dest)
...
if __name__ == "__main__":
    ...
    con, sql_q10 = prepare_duckdb(filtered_orders, QUERY_PATH)
    engine_metrics_duck = measure_query_duckdb(10, con, sql_q10)
    ...
    ctx, sql_q10_df = prepare_datafusion(filtered_orders, QUERY_PATH)
    engine_metrics_df = measure_query_datafusion(10, ctx, sql_q10_df)
