...
def process_nation_parquet(file_path, batch_size=100000):
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        ...
        filtered_batches.append(df_batch[["n_nationkey", "n_regionkey", "n_name"]])
        global_offset += n_rows
        del df_batch

    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, n_nationkey_index, n_regionkey_index, n_name_index
...
def process_part_parquet(file_path, batch_size=100000):
    ...
    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, p_type_index, p_partkey_index
...
def process_lineitem_parquet(file_path, batch_size=100000):
    ...
    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, l_partkey_index, l_suppkey_index, l_orderkey_index, l_extendedprice_index, l_discount_index
...
if __name__ == "__main__":
    ...
    nation_df, n_nationkey_index, n_regionkey_index, n_name_index = process_nation_parquet(nation_parquet)
    part_df, p_type_index, p_partkey_index = process_part_parquet(part_parquet)
    supplier_df, s_suppkey_index, s_nationkey_index = process_supplier_parquet(supplier_parquet)
    lineitem_df, l_partkey_index, l_suppkey_index, l_orderkey_index, l_extprice_index, l_discount_index = process_lineitem_parquet(lineitem_parquet)
    ...
    bitmap_size_mb = bitmap_memory_size(
        r_name_index,
        n_nationkey_index, n_regionkey_index, n_name_index,
