...
def process_customer_parquet(file_path, batch_size=100000):
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        ...
        for col in ["c_custkey", "c_nationkey"]:
            if col in df_batch.columns:
                original_size_bytes += df_batch[col].memory_usage(deep=True)
...
def process_orders_parquet(file_path, batch_size=100000):
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        ...
        if "o_orderkey" in df_batch.columns:
            original_size_bytes += df_batch["o_orderkey"].memory_usage(deep=True)
...
def process_lineitem_parquet(file_path, batch_size=100000):
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        ...
        for col in ["l_orderkey", "l_suppkey", "l_extendedprice", "l_discount"]:
            if col in df_batch.columns:
                original_size_bytes += df_batch[col].memory_usage(deep=True)
    ...
    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, l_orderkey_index, l_suppkey_index, l_extendedprice_index, l_discount_index, original_size_bytes, bitmap_creation_time

def prepare_duckdb(region_df, nation_df, customer_df, orders_df, supplier_df, lineitem_df, query_file):
...
def prepare_datafusion(region_df, nation_df, customer_df, orders_df, supplier_df, lineitem_df, query_file):
    if "l_extendedprice" in lineitem_df.columns:
        lineitem_df["l_extendedprice"] = lineitem_df["l_extendedprice"].astype("float64")
    if "l_discount" in lineitem_df.columns:
        lineitem_df["l_discount"] = lineitem_df["l_discount"].astype("float64")
...
if __name__ == "__main__":
    ...
    lineitem_df, l_orderkey_index, l_suppkey_index, l_extendedprice_index, l_discount_index, lineitem_orig_size, lineitem_bitmap_time = process_lineitem_parquet(lineitem_parquet)
    ...
    total_original_bytes = (region_orig_size + nation_orig_size + customer_orig_size +
                            orders_orig_size + supplier_orig_size + lineitem_orig_size)
    original_size_mb = total_original_bytes / (1024.0 * 1024.0)
    
    total_bitmap_time = (region_bitmap_time + nation_bitmap_time + customer_bitmap_time +
                         orders_bitmap_time + supplier_bitmap_time + lineitem_bitmap_time)
