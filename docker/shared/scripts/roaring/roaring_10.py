...
def process_customer_parquet(file_path, batch_size=100000):
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        ...
        keep_cols = [
            ...
            "c_address", "c_phone", "c_comment", "c_nationkey"
        ]
...
def process_orders_parquet(file_path, batch_size=100000):
    ...
    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, o_orderkey_index, o_custkey_index, o_orderdate_index
...
def process_lineitem_parquet(file_path, batch_size=100000):
    ...
    for batch in pf.iter_batches(batch_size=batch_size):
        ...
        mask = (df_batch["l_returnflag"] == desired_flag)
    ...
    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, l_orderkey_index, l_extendedprice_index, l_discount_index, l_returnflag_index
...
def process_nation_parquet(file_path, batch_size=100000):
    ...
    filtered_df = pd.concat(filtered_batches, ignore_index=True) if filtered_batches else pd.DataFrame()
    del filtered_batches
    return filtered_df, n_nationkey_index, n_name_index
...
if __name__ == "__main__":
    ...
    orders_df, o_orderkey_index, o_custkey_index, o_orderdate_index = process_orders_parquet(orders_parquet)
    lineitem_df, l_orderkey_index, l_extprice_index, l_discount_index, l_returnflag_index = process_lineitem_parquet(lineitem_parquet)
    nation_df, n_nationkey_index, n_name_index = process_nation_parquet(nation_parquet)

    bitmap_size_mb = bitmap_memory_size(
        c_custkey_index, c_nationkey_index,
        o_orderkey_index, o_custkey_index, o_orderdate_index,
